package sistemaDeGestion.Negocio;
...
import sistemaDeGestion.Helper.Utility;
...
public class EmpleadoBussines {
    ...
    private boolean cumpleCriterio(Empleado empleado, int criterio, String valorString, double valorMinimo, double valorMaximo) {
        return switch (criterio) {
            case Utility.CRITERIO_NOMBRE ->
                empleado.getNombre().equalsIgnoreCase(valorString);
            case Utility.CRITERIO_EDAD ->
                empleado.getEdad() >= valorMinimo && empleado.getEdad() <= valorMaximo;
            case Utility.CRITERIO_SALARIO ->
                empleado.getSalario() >= valorMinimo && empleado.getSalario() <= valorMaximo;
            case Utility.CRITERIO_DEPARTAMENTO ->
                empleado.getDepartamento().equalsIgnoreCase(valorString);
            default ->
                false;
        };
    }
    ...
    public List<Empleado> ordenarEmpleados(List<Empleado> empleados, int criterio) {
        Comparator<Empleado> comparator = switch (criterio) {
            case Utility.CRITERIO_NOMBRE ->
                Comparator.comparing(Empleado::getNombre, String.CASE_INSENSITIVE_ORDER);
            case Utility.CRITERIO_EDAD ->
                Comparator.comparingInt(Empleado::getEdad);
            case Utility.CRITERIO_SALARIO ->
                Comparator.comparingDouble(Empleado::getSalario);
            case Utility.CRITERIO_DEPARTAMENTO ->
                Comparator.comparing(Empleado::getDepartamento, String.CASE_INSENSITIVE_ORDER);
            default ->
                throw new IllegalArgumentException("Criterio no valido");
        };
